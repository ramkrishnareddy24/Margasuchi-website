					...
					<div class="ps-3">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb mb-0 p-0">
								...
								<li class="breadcrumb-item active" aria-current="page">Un Paid payments</li>
							</ol>
						</nav>
					</div>
					...
					<div class="card-body">
						<div class="table-responsive">
							<table id="example2" class="table table-striped table-bordered">
								...
								<?php 
								include('config.php');
								$query=mysqli_query($con,"select * from payments where status=0 order by id");
								while($row=mysqli_fetch_array($query))
								{
								?>
									<tr>
									     <td> <a href="manage_payments.php?id=<?php echo $row['id']; ?>"><button class="btn btn-primary">PAID</button></a> </td>
...
if (isset($_GET['id'])) {
  ...
  $getch=mysqli_query($con,"select * from payments where id='$key'");
  $row=mysqli_fetch_array($getch);
  $rand=$row['rand'];
  $update=mysqli_query($con,"UPDATE ref set status=1 where rand='$rand'");
  echo "<meta http-equiv=\"refresh\" content=\"0;URL=manage_payments.php\">";
}
?>
<script>
